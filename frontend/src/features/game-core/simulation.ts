...
import type { MoveInput, SimulationConfig, WorldMap, WorldState, Vector2 } from './contracts';
import { assertValidWorldMap } from './map-validation';
import { moveCircle } from './collision';
import { createColliderIndex } from './spatial-hash';
...
export function simulateTick(
  ...
  const index = createColliderIndex(map.colliders, Math.max(config.maxSubstepDistance, 1));
