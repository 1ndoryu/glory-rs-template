 ...
 *  de inserción se conserva: las pinceladas nuevas van al final, lo que hace
 *  predecible la búsqueda de `containsCell` (recorre desde el final). */
...
function shapeSdf(
  ...
  switch (shape.kind) {
    ...
    case 'painted':
      ...
      return containsCell(shape.cells, cellI, cellJ) ? -1 : Infinity;
  }
}

function containsCell(cells: readonly (readonly [number, number])[], i: number, j: number): boolean {
  /* Busqueda lineal acotada: la cuota de celdas (16k) limita el costo y el
   * stack se aplica por capa sobre su AABB. Para pinceladas largas el panel
   * mantiene orden de inserción y las pinceladas nuevas van al final. */
  for (let k = cells.length - 1; k >= 0; k -= 1) {
    if (cells[k][0] === i && cells[k][1] === j) return true;
  }
  return false;
}
...
function shapeCellBounds(
  ...
  switch (shape.kind) {
    case 'circle':
      absorb(shape.cx - shape.radius, shape.cz - shape.radius);
      absorb(shape.cx + shape.radius, shape.cz + shape.radius);
      break;
    case 'curve':
    case 'polygon':
      for (const [x, z] of shape.points) absorb(x, z);
      break;
    case 'painted':
      for (const [i, j] of shape.cells) absorb((i + 0.5) * cellSize - halfX, (j + 0.5) * cellSize - halfZ);
      break;
  }
...
export function applyTerrainLayerStack(
  ...
  for (const layer of layers) {
    ...
    const bounds = shapeCellBounds(layer.shape, width, depth, cellSize);
...
function applyLayerRegion(
  ...
  for (let j = bounds.minJ; j <= bounds.maxJ; j += 1) {
    for (let i = bounds.minI; i <= bounds.maxI; i += 1) {
      ...
      const d = shapeSdf(layer.shape, worldX, worldZ, i, j);
