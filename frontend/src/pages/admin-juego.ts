 ...
 * [297A-61] asset.created/asset.updated del catálogo de assets. */
...
/** Liberar todas las listas del catálogo (personajes y assets) de una página
 * Admin en un solo recorrido: cada lista tiene su propio WeakMap de guard. */
export function disposeAdminGameCharacterLists(page: HTMLElement): void {
  page.querySelectorAll<HTMLElement>('.admin-lista').forEach((element) => {
    disposeGameCharacterAdminList(element);
    disposeGameAssetAdminList(element);
  });
}
...
export async function renderGameCharacterAdminList(container: HTMLElement): Promise<void> {
   ...
   * [297A-61] La actividad de assets se carga en el propio render de la lista
   * de assets (mismo patrón de carga paralela aislada). */
  ...
  container.appendChild(renderActividad(auditResult, 'actividad del catálogo'));
  container.appendChild(renderActividadMapas(mapAuditResult));
}

/* === Catálogo de assets del juego (297A-61) ===
 * Replica el patrón del catálogo de personajes con el contrato de 297A-60:
 * lista activas e inactivas, alta (id + etiqueta + categoría) y edición con
 * estado. El alta vive en el modal y el botón lo abre desde la franja. */

/** Renderiza el listado completo de assets (activas e inactivas) con guard
 * de generación propio y actividad aislada en paralelo. */
export async function renderGameAssetAdminList(container: HTMLElement): Promise<void> {
  ...
  const items = result.value;
  for (const item of items) {
    container.appendChild(renderAdminAssetItem(item, container));
  }
  if (items.length === 0) {
    container.appendChild(createVacio('no hay assets en el catálogo'));
  }
  container.appendChild(renderActividad(auditResult, 'actividad de assets'));
}

/** Liberar el guard de generación de una lista de assets. */
export function disposeGameAssetAdminList(container: HTMLElement): void {
...
/** Sección de actividad reciente (últimos eventos auditados). El título lo
 * fija el llamador: "actividad del catálogo" (personajes) o "actividad de
 * assets" (297A-61); el payload de ambos eventos expone displayName. */
function renderActividad(result: { ok: true; value: GameAuditEventEntry[] } | { ok: false; error: string }, titulo: string): HTMLElement {
  const seccion = createEl('section');
  seccion.appendChild(createEl('h3', { className: 'mt-lg mb-md', textContent: titulo }));
  if (!result.ok) {
...
function renderAdminAssetItem(entry: GameAssetAdminEntry, container: HTMLElement): HTMLElement {
  const tag = createEl('span', {
    className: 'tag-estado',
    textContent: entry.isActive ? 'activo' : 'inactivo',
  });
  ...
  editButton.addEventListener('click', () => openEditarAssetModal(entry, () => {
    void renderGameAssetAdminList(container);
  }));
  ...
  toggleButton.addEventListener('click', () => {
    /* [297A-61] Mismo criterio que personajes: desactivar pide confirmación
     * (desaparece del Editor de mapa), reactivar va directo. */
    void safeRun((async () => {
      if (entry.isActive) {
        const confirmed = await showConfirm(`desactivar "${entry.displayName}"?`);
        if (!confirmed) return;
...
/** Modal de alta: id (slug allowlisted) + etiqueta + categoría; nace activo. */
...
/** Modal de edición: etiqueta + categoría + estado activo (el id es inmutable). */
