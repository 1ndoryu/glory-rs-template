...
export async function openEditor(article?: Article): Promise<void> {
    ...
    const toolbarButtons: Array<{label: string; action: () => void}> = [
        ...
        },
        {
            ...
            action: async () => {
                try {
                    const r = await pickAndUpload('image/*', article?.id);
        ...
        },
        {
            ...
            action: async () => {
                try {
                    const r = await pickAndUpload('audio/*', article?.id);
                    if (r) editor.chain().focus().insertContent(`<audio controls src="${r.url}"></audio>`).run();
                } catch {
        ...
        },
        {
            ...
            action: async () => {
                try {
                    const r = await pickAndUpload('video/*', article?.id);
                    if (r) editor.chain().focus().insertContent(`<video controls src="${r.url}" style="width:100%"></video>`).run();
                } catch {
                    showToast('error al subir video');
                }
            }
        }
    ];
    ...
    coverBtn.addEventListener('click', async () => {
        try {
            ...
            showToast('error al subir portada');
        }
    });
