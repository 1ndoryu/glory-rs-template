...
- **028A-6 Fase 1 (módulos 1–8, 2026-08-05):** los módulos del orquestador fueron extraídos a Sentinel Core sin imports del proyecto ni de VarSense: `scope.ts`, `gateReport.ts` + `redaction.ts`, `stageCache.ts`, `scheduler.ts`, `policyDecision.ts`, `toolRunner.ts` (procesos con env allowlist/captura acotada/timeout/cancelación), `stageRunner.ts` (concurrencia con drenaje) y `structuredTool.ts` (contrato JSON versionado con estados fail-closed). El CLI gana `sentinel check <task-id> --dry-run` (alcance + guard), `sentinel guard` (comando directo v2/legacy, exit 78), `sentinel doctor`/`status` (diagnóstico read-only) y `sentinel check <task-id> --stages <json>` que ejecuta el gate real (alcance → caché → runner → reporte combinado con exit code). Orquestación en `core/gateRun.ts`; CLI delgado con SIGINT→cancelAll. Upstream `a57cfc1` (origin/main); **371 tests PASS** (29 nuevos). La migración es incremental: `scripts/quality` sigue siendo el gate hasta la integración del orquestador de wandori.us a `--stages` (Fase 3/observe).
