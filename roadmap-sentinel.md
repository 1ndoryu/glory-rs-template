...
| VarSense | `2.2.0` / `4167868dd5d0e7674d5565ade399a57796d69cf3` | Commit reproducible consumido por el gate. `main` local además contiene SNT-08 (`337c4cce`), aún no publicado ni fijado en la instalación para no romper checkouts limpios. |
...
- [ ] Invalidar índices por archivo y dependencias, no por workspace completo.
- [x] Añadir cancelación cooperativa editor-agnostic en `main` de VarSense para builders de variables/clases, con propagación estable y regresiones de cancelación/errores normales (commit local `337c4cce`; 50 tests upstream PASS). La fijación en `.quality-tools` queda pendiente de publicación en `origin`.
