...
- **028A-6 Fase 1 (módulos 1–5, 2026-08-05):** los cinco módulos puros del orquestador fueron extraídos a Sentinel Core sin imports del proyecto ni de VarSense: `scope.ts` (alcance incremental), `gateReport.ts` + `redaction.ts` (reporte combinado), `stageCache.ts` (fingerprint de PASS por modo/runtime/política/lock/contenido), `scheduler.ts` (cooldown de full + lock activo, raíz por `sentinel.config.json`/`quality.config.json`) y `policyDecision.ts` (clasificador v1/v2). El CLI gana `sentinel check <task-id> --dry-run [--full|--ci|--allow-heavy]`: escribe `changed-files.txt` + `scope-manifest.json`, consulta el guard para decidir el full diferido (`heavyGuard` en la salida) y devuelve JSON; exige `--dry-run` para no simular un gate que aún no existe. Upstream publicado en `7f5b086` (origin/main); **342 tests de la suite PASS** (14 nuevos). La migración es incremental: `scripts/quality` sigue siendo el gate hasta que se extraiga el runner de etapas y se integre check→etapas→reporte.
...
- Pendiente de 028A-8: watchers/LSP persistentes, índices globales de Sentinel, presupuestos p50/p95 y benchmark; pendiente decidir si Sentinel migra al mismo patrón de submódulo.
...
- [ ] Extraer scheduler, cooldown, locks, scope, caché y reporter desde `scripts/quality` al runtime de Sentinel; conservar `task:check` como alias temporal. *(módulos puros extraídos al core en `7f5b086`; runner de etapas e integración check→etapas→reporte pendientes)*
