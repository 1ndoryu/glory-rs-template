...
function assertName(value, label) {
  if (typeof value !== 'string' || value.length === 0 || value.length > 128 || !/^[A-Za-z0-9:_*.-]+$/u.test(value)) {
    throw new Error(`${label}: nombre inválido`);
  }
}

export function assertTaskId(value, label = 'task-id') {
  if (typeof value !== 'string' || !TASK_ID_PATTERN.test(value)) throw new Error(`${label}: identificador inválido`);
  return value;
}
...
export function resolveWorkspacePath(projectRoot, candidate, label, { allowReportRoot = false } = {}) {
  ...
  if (allowReportRoot && !relative.startsWith(`.quality-reports${path.sep}`) && relative !== '.quality-reports') {
    throw new Error(`${label}: debe permanecer bajo .quality-reports`);
  }
...
function assertRelativePath(value, label) {
  if (typeof value !== 'string' || value.length === 0 || path.isAbsolute(value)) throw new Error(`${label}: debe ser una ruta relativa`);
  if (value.replace(/\\/g, '/').split('/').includes('..')) throw new Error(`${label}: no puede salir del workspace`);
}

function assertStageList(value, label, declaredStages) {
  if (!Array.isArray(value) || value.length === 0) throw new Error(`${label}: debe ser una lista no vacía`);
  for (const stage of value) {
    assertName(stage, label);
    if (!declaredStages.has(stage)) throw new Error(`${label}: etapa desconocida ${stage}`);
  }
}

export function manifestStageNames(manifest) {
  if (!isRecord(manifest?.stages) || Object.keys(manifest.stages).length === 0) throw new Error('quality-adapter.json.stages: debe ser un objeto no vacío');
  return Object.keys(manifest.stages);
}
...
export function validateAdapterManifest(manifest) {
  ...
  const sensitive = manifest.adapter.environment.allowlisted.filter(name => SENSITIVE_ENV_NAMES.test(name));
  if (sensitive.length > 0) throw new Error(`quality-adapter.json.adapter.environment.allowlisted: variables sensibles no permitidas (${sensitive.join(', ')})`);
  ...
  for (const [name, expected] of Object.entries({ pass: 0, findings: 1, toolError: 2, cancelled: 130 })) {
    if (manifest.adapter.output.exitCodes[name] !== expected) throw new Error(`quality-adapter.json.adapter.output.exitCodes.${name}: debe ser ${expected}`);
  }
  ...
  for (const argument of manifest.transport.arguments) {
    if (typeof argument !== 'string' || (!PLACEHOLDERS.has(argument) && argument.includes('{'))) throw new Error('quality-adapter.json.transport.arguments: placeholder no permitido');
  }
  ...
  if (!isRecord(manifest.stages) || Object.keys(manifest.stages).length === 0) throw new Error('quality-adapter.json.stages: debe ser un objeto no vacío');
  const declaredStages = new Set(manifestStageNames(manifest));
  ...
  for (const [stage, definition] of Object.entries(manifest.stages)) {
    ...
    if (!isRecord(definition) || !Number.isInteger(definition.timeoutMs) || definition.timeoutMs < 1 || definition.timeoutMs > 30 * 60 * 1000) throw new Error(`quality-adapter.json.stages.${stage}.timeoutMs: inválido`);
  }

  if (!isRecord(manifest.profiles)) throw new Error('quality-adapter.json.profiles: debe ser un objeto');
  for (const [profile, stages] of Object.entries(manifest.profiles)) {
    assertName(profile, `quality-adapter.json.profiles.${profile}`);
    assertStageList(stages, `quality-adapter.json.profiles.${profile}`, declaredStages);
  }
  return manifest;
}

export async function readAdapterManifest(projectRoot) {
  const manifestPath = path.join(projectRoot, ADAPTER_MANIFEST_FILE);
  let manifest;
  ...
  try {
    if (!statSync(entrypointPath).isFile()) throw new Error('no es un archivo regular');
...
export function adapterStageNames(manifest, profiles, full) {
  const declared = manifestStageNames(manifest);
  if (full) return declared;
  const selected = new Set(REQUIRED_STAGE_NAMES);
  for (const profile of profiles) {
    if (!Object.hasOwn(manifest.profiles, profile)) throw new Error(`Perfil de adapter desconocido: ${profile}`);
    for (const stage of manifest.profiles[profile]) selected.add(stage);
  }
  return declared.filter(stage => selected.has(stage));
}

export function assertImplementedStages(manifest, selectedStages, implementedStages) {
  const implemented = new Set(implementedStages);
  for (const stage of selectedStages) if (!implemented.has(stage)) throw new Error(`Etapa declarada por el adapter sin implementación: ${stage}`);
  return manifest;
}

export function assertStageParity(selectedStages, implementedStages) {
  const selected = [...new Set(selectedStages)];
  const implemented = [...new Set(implementedStages)];
  if (selected.length !== implemented.length || selected.some((stage, index) => stage !== implemented[index])) throw new Error(`Deriva de etapas entre manifest y adapter: manifest=${selected.join(',')} implementation=${implemented.join(',')}`);
  return true;
}

export function materializeTransportArguments(manifest, values) {
  const replacements = { '{stage}': values.stage, '{reportPath}': values.reportPath, '{taskId}': values.taskId };
  return manifest.transport.arguments.map(argument => {
    if (!PLACEHOLDERS.has(argument)) return argument;
    const value = replacements[argument];
    if (typeof value !== 'string' || value.length === 0) throw new Error(`Falta valor para ${argument}`);
    return value;
  });
}

export function adapterEnvironmentAllowlist(manifest, defaultAllowlist = []) {
  validateAdapterManifest(manifest);
  return [...new Set([...(defaultAllowlist ?? []), ...manifest.adapter.environment.allowlisted])];
}
