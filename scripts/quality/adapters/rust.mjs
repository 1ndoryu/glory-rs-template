...
export async function runRust(context, scope = {}) {
  ...
  const check = await runStep(context, 'cargo check', process.execPath, [runWithDb, 'check']); steps.push(check);
  ...
  return {
    stage: 'rust', status: infrastructure ? 'error' : failed.length > 0 ? 'fail' : 'pass', durationMs: Date.now() - startedAt,
    validationMode: runExtendedChecks ? 'full' : 'local-light',
    findings: failed.map(step => ({ ruleId: step.execution.timedOut ? 'quality-timeout' : `rust-${step.name.split(' ')[1] ?? 'command'}`, severity: 'error', message: conciseFailure(`${step.execution.stdout}\n${step.execution.stderr}`, `${step.name} falló`) })),
    summary: failed.length > 0 ? `${failed.length} comandos fallaron` : runExtendedChecks ? `${steps.length} comandos pasaron` : `${steps.length} comandos pasaron (clippy/tests reservados para --full)`, logPath,
  };
}
