...
test('poda por cuota los targets más viejos', async () => {
  ...
  try {
     ...
     * b reciente (protegido por escritura reciente): juntos superan 1 GB. */
...
test('la cuota no toca targets con escritura reciente (cargo en compilación)', async () => {
  const projectRoot = await mkdtemp(path.join(os.tmpdir(), 'quality-target-recent-'));
  const { base, targetRoot } = makeTargetRoot();
  const now = Date.now();
  try {
    await projectConfig(projectRoot);
    /* [028A-6] cargo/rustc en compilación no cuelgan del target por ruta de
     * ejecutable (viven en .rustup): el mtime reciente es la única señal. */
...
test('el throttle del gate evita pasos repetidos y el fallo no rompe el best-effort', async () => {
  ...
  try {
    ...
    /* El best-effort respeta el throttle y devuelve skipped. */
    const skipped = await runTargetMaintenanceBestEffort({
      projectRoot,
      targetRoot,
      now: now + 60_000,
