...
> **Reauditoría de seguimiento — 2026-08-12, corte final:** se repitió el preflight, el gate y la
...
**Veredicto actual:** `OK CON RETIRADA CONDICIONADA`. El contrato local está listo y reproducible con Sentinel
...
| Suite adapter wandorius | `quality:test`: 251 tests, 250 PASS, 1 omitido, 0 fallos, 18.5 s | PASS |
...
| `npm run quality:test` | PASS | 250 PASS, 1 omitido, 0 fallos, 18.5 s; los fixtures de cuota ya no escriben cientos de MB/GB por test. |
...
La suite del consumidor cerró en 18.5 s (251 tests: 250 PASS, 1 omitido). Sigue siendo más pesada que un
...
| Segunda release verde + rollback | **Completado con límites explícitos:** #45/#46 verdes, matriz focal y gates pasan, rollback histórico fue probado; falta la pre-verificación PATH sin runtime de desarrollo. |
| Retirada física de capas A/B | **Capa A pendiente condicionada:** solo shims/wrappers duplicados tras §3; capa B permanece hasta SNT-10. |
...
| Suite completa de calidad | **Completada:** 251 tests, 250 PASS, 1 omitido, 0 fallos, 18.5 s; los fixtures de cuota ya no consumen GB reales. |
...
- [ ] Completar PATH sin runtime de desarrollo, smoke de enforcement y rollback de salida; retirar únicamente la capa A si todas las pruebas pasan.
...
- [ ] Eliminar solo wrappers duplicados con paridad demostrada, PATH sin runtime de desarrollo y rollback preparado.
...
**Conclusión:** la instalación, bootstrap, readiness, lock, rendimiento y adopción multi-consumidor quedan corregidos en Sentinel `0.7.4`/VarSense `2.2.1`; el rollback real también está verificado. Sentinel y gate son una sola autoridad de decisión (`gate:check` prepara; `sentinel check` decide), mientras `task:check` queda como alias de compatibilidad. No se deben borrar ni copiar carpetas personalizadas: la capa A solo se retira después de completar PATH sin runtime de desarrollo y rollback de salida; la capa B permanece hasta SNT-10. El baseline `broadcast-mutex-riesgo-rs` de glory-rs-rest queda como deuda de producto separada y visible, no como falso PASS.
