...
- Quality gate Sentinel/VarSense: Sentinel 0.7.4/VarSense 2.2.1 publicados y adoptados; `gate:check` delega en `sentinel check`; rollback 0.7.1 ↔ 0.7.0 verificado; CI #45/#46 y matriz focal verdes; retirada física de la capa A aún condicionada a PATH sin runtime de desarrollo, enforcement y rollback de salida. La capa B espera SNT-10.
...
- Migración global 028A-6/108A-6: Sentinel 0.7.4/VarSense 2.2.1, lock/doctor y gates de consumidores verificados; CI Ubuntu #45/#46 y matriz focal verdes; pendiente solo la pre-verificación de retirada de wrappers y el baseline de producto `broadcast-mutex-riesgo-rs` de glory-rs-rest (warning visible).
