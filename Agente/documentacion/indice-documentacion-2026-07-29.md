...
- Quality gate Sentinel/VarSense: Sentinel 0.7.2/VarSense 2.2.1 publicados y en adopción; `gate:check` delega en `sentinel check`; rollback 0.7.1 ↔ 0.7.0 verificado; retirada física legacy condicionada a dos CI consecutivos, matriz multi-shell y gates verdes.
...
- Migración global 028A-6/108A-6: Sentinel 0.7.2/VarSense 2.2.1, lock/doctor y adopción de consumidores en verificación; pendiente la CI Ubuntu, retirada física condicionada y el baseline de producto `broadcast-mutex-riesgo-rs` de glory-rs-rest (ahora warning visible).
