...
### Fase 1 — Sentinel Core global instalable y estable *(bloqueada: runtime upstream ausente; los 5 módulos del orquestador ya están en el core)*

**Avance 2026-08-05 (módulos 1–5):** los cinco módulos puros del orquestador fueron extraídos a Sentinel Core sin imports de wandori.us ni de VarSense (verificado por `check:core`): `src/core/scope.ts` (port de `scope.mjs`: `resolveFullDecision`, globs, `resolveExplicitProfiles`, `expandLocalDependencies` y `detectScope` con git); `src/core/gateReport.ts` + `src/core/redaction.ts` (ports de `reporter.mjs`/`redaction.mjs`: decisión PASS/FAIL/SETUP-ERROR/CANCELLED 130, orden estable, `latest.json`/`latest.md`, límites 3/4 y redacción de secretos); `src/core/stageCache.ts` (port de `cache.mjs`: fingerprint v5 por modo efectivo/runtime/config/política/lock/contenido); `src/core/scheduler.ts` (port de `heavy-run-guard.mjs`: cooldown de full + lock activo `wx` con raíz por marcador `sentinel.config.json`/`quality.config.json` y fusión de `heavyRun` v2??v1 en migración; el wrapper `--execute-cargo` queda en el orquestador, es integración de migración); `src/core/policyDecision.ts` (clasificador puro `no-policy/legacy-v1/policy/invalid-policy`). El CLI gana `sentinel check <task-id> --dry-run [--full|--ci|--allow-heavy]`: calcula el alcance, escribe `changed-files.txt` + `scope-manifest.json`, consulta el guard para decidir el full diferido (`heavyGuard` en la salida) y devuelve JSON; exige `--dry-run` para no simular un gate que aún no existe. `writeAtomic` pasa a utilidad compartida (`atomicFile.ts`). El checkout consumido es el submódulo interno `tools/sentinel` y el gate no depende de variables `GLORY_*`. Evidencia: 342 tests de la suite upstream PASS (14 nuevos: 3 de cache, 7 de scheduler con repo real, symlinks, cooldown por proyecto y mensajes de guard, 4 de política), compile + `check:core` PASS, y fijación en `quality-tools.json`/`sentinel.lock.json` (commits `5a968c8` → `7f5b086`). Pendiente de esta fase: runner de etapas (integración check→etapas→reporte), resto del CLI global y shims.

- [ ] Extraer el clasificador, scheduler, scope, caché y reporter a Sentinel Core, sin imports de wandori.us ni de VarSense. *(los 5 módulos extraídos en `5a968c8` → `7f5b086`; el runner de etapas y la integración check→etapas→reporte siguen pendientes)*
- [ ] Crear CLI global `sentinel check|guard|doctor|status|install|update|rollback`. *(check --dry-run con scope + guard de cooldown ya existe; guard/doctor/status/install pendientes)*
