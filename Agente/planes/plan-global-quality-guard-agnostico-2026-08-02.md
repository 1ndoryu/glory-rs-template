...
- [ ] Eliminar shims duplicados del repositorio cuando dos versiones consecutivas hayan pasado la matriz. *(runbook listo en `Agente/documentacion/herramientas/runbook-retirada-wrappers-sentinel-2026-08-05.md`: criterio operativo de las dos releases, pre-verificación, `git rm` exacto de la capa A — shims del repo + `quality-command-guard.mjs` y su test, que no tiene dependientes fuera de la capa A —, verificación post-retirada con bloqueo 78 del runtime y rollback. La capa B — `task:check`/`heavy-run-guard` — se retira aparte, en el gate SNT-10, cuando `sentinel check` sea la única autoridad)*
