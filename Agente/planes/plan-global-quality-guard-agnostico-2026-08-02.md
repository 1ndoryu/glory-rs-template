...
- [x] VarSense no crea cooldown, lock ni reporte paralelo: el adapter ejecuta una sola invocación `all`, usa el `reportRoot`/logs del gate y devuelve hallazgos mediante `structured-tool`; la metadata `varsenseScope` conserva el manifiesto solicitado. La integración versionada solo añade `--files-from` cuando `quality-tools.json.tools.varsense.capabilities.filesFrom=true`; esa capacidad también queda ligada a `sentinel.lock.json`. La instalación fijada continúa sin capacidad y reporta `varsense-cli-{version}-no-files-from`; publicar/fijar `858ec62` sigue pendiente.
...
- [ ] Publicar/fijar `858ec62`, declarar `capabilities.filesFrom` en `quality-tools.json` y regenerar `sentinel.lock.json`; después ejecutar el gate real en local-light y full/CI.
