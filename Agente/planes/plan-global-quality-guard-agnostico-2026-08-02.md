...
- [ ] Crear CLI global `sentinel check|guard|doctor|status|install|update|rollback`. *(check con scope/guard/etapas reales, guard, doctor y status ya existen; install/update/rollback pendientes del runtime global)*
