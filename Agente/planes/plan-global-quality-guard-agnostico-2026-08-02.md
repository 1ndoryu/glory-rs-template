...
- [x] Cubrir con fixture de integración el aislamiento de dos ramas para reportes, caché y locks, además de identidades CI/detached con commit compartido (`scripts/quality/tests/branch-isolation.integration.test.mjs`). Traversal/symlink, retención y poda tienen cobertura focal; el cambio de rama sin reinicio, runner CI reutilizado, fallo no bloqueante integrado en el gate y la prueba multi-proceso/CI real siguen pendientes del runtime global.
