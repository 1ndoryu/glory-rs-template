...
**Avance 2026-08-05 (módulos 1 y 2):** el alcance incremental y el reporte combinado del orquestador fueron extraídos a Sentinel Core. `src/core/scope.ts` (port agnóstico de `scripts/quality/scope.mjs`: `resolveFullDecision`, globs, `resolveExplicitProfiles`, `expandLocalDependencies` y `detectScope` con git vía `execFile`); el CLI gana `sentinel check <task-id> --dry-run` (calcula el alcance efectivo, escribe `changed-files.txt` + `scope-manifest.json` y devuelve JSON; exige `--dry-run` para no simular un gate que aún no existe). `src/core/gateReport.ts` + `src/core/redaction.ts` (ports de `reporter.mjs`/`redaction.mjs`: decisión PASS/FAIL/SETUP-ERROR/CANCELLED 130, orden estable de hallazgos, `latest.json`/`latest.md`, límites compactos 3/4 y redacción de secretos; el comando recomendado del runtime agnóstico pasa a `sentinel check <task-id>`). El checkout consumido pasó a ser el submódulo interno `tools/sentinel` (mismo patrón que `tools/varsense`) y el gate ya no depende de variables `GLORY_*`. Evidencia: 327 tests de la suite upstream PASS (7 nuevos de `gateReport` + 5 de `scope`, incluidos repo git real y secretos), compile + `check:core` PASS, y fijación en `quality-tools.json`/`sentinel.lock.json` (commits `5a968c8` → `7eee63e`). Pendiente de esta fase: scheduler/cooldown, caché de etapas, clasificador, resto del CLI global y shims.

- [ ] Extraer el clasificador, scheduler, scope, caché y reporter a Sentinel Core, sin imports de wandori.us ni de VarSense. *(scope y reporter extraídos en `5a968c8`+`7eee63e`; scheduler/caché/clasificador pendientes)*
