...
> **Fecha:** 2026-08-06
> **Estado:** Fase 2 local de transición parcialmente implementada; schema upstream, fixtures multi-proyecto, paridad, rollback y retirada física siguen pendientes
...
El problema no es simplemente que haya muchos archivos. El problema es que el checkout mezcla cuatro responsabilidades con vidas útiles distintas: plano universal, adaptación por proyecto, compatibilidad temporal y operación específica/histórica.
...
### Fase 2 — Core/adapter slice (`SNT-14`) — parcialmente cerrada en transición local

**Implementado en el consumidor:**
...
- `runner.mjs` hereda únicamente una allowlist base no sensible; el manifest también rechaza nombres sensibles (`DATABASE_URL`, tokens, keys, passwords y equivalentes), por lo que no se transportan credenciales por declaración implícita;
...
- tests dirigidos de contrato, runner, stages y wrappers cubren desconocidos, paths, symlink escape, allowlist y exit codes.

**Evidencia 2026-08-06:** suite dirigida de SNT-14 = **19 tests, 18 PASS, 1 skip, 0 fail**; el skip corresponde al observe end-to-end sin CLI Sentinel provisionado/limpio. `node --check` y `git diff --check` PASS. El gate del consumidor se ejecutó, pero no pasa en este worktree: falta `.env`/`DATABASE_URL` y faltan dependencias frontend (`MODULE_NOT_FOUND`); no se copian secretos ni se falsea el resultado. El submódulo Sentinel y VarSense quedan limpios en sus commits fijados.

**No implementado todavía:** schema upstream, fixtures Node/Vite + Rust/PostgreSQL independientes, cambios upstream publicados, paridad CLI/LSP/editor, cinco tareas reales, métricas comparables RSS/CPU, rollback y releases consecutivos.

### Fase 3 — Reducir el adapter de wandori.us (`SNT-15`)
...
### Fase 4 — Retirada controlada y simplificación (`SNT-16`)
...
- [x] documentación local refleja el estado real de Fase 1/Fase 2;
