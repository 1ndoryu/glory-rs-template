...
- [ ] **297A-30 — Publicación admin versionada de MapVersion:** `POST /api/admin/game/maps` con `AdminUser`/CSRF, `expectedVersion`, canonicalización/hash, advisory lock, activación atómica, límite de body de 4 MiB y request OpenAPI. Pendiente: fixture persistido y pruebas HTTP/DB reales de autorización, 413, stale revision, concurrencia y una sola versión activa; sin realtime, identidad invitada ni editor.
