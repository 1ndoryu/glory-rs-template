...
- [ ] **028A-15 — Ejecutar los tests de integración del guard de publish (028A-11):** la implementación del guard quedó cerrada con gate local-light PASS, pero `cargo test --test workspace_publish` no se ejecutó porque el cooldown de 180 min del guard de ejecuciones pesadas lo bloqueó y el usuario desaprobó saltarlo. Cuando expire el cooldown, ejecutar `npm run task:check -- 028A-11 --full` (o `cargo test --test workspace_publish`) y confirmar los 4 tests de `tests/workspace_publish.rs`; actualizar esta entrada y completados con el resultado.
