...
- [ ] **297A-28 — Lectura pública persistida de MapVersion:** migración `game_map_versions`, snapshot JSONB inmutable con límite de tamaño, hash SHA-256, índice de una versión activa, repository/service/handler `GET /api/game/maps/:map_id` y envelope público sin campos administrativos. Gate PASS: Sentinel/VarSense, `cargo check`, 11 tests `models::game_map`, type-check, 23 tests frontend, build y diff-check. Pendiente: publicación admin y fixture persistido; sin realtime, identidad ni editor.
