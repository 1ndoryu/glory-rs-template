...
> **Toma de tareas (obligatoria):** antes de empezar una tarea, marcarla con `npm run task:take -- --task {ID} --by <agente>`. El identificador devuelto (`T-<epochMs>-<hex>`) codifica el instante exacto de la toma, así que basta leerlo para saber cuándo se tomó. Una tarea ya tomada por otro agente activo NO se empieza: `task:take` la rechaza con conflicto, y `npm run task:status` lista todas las tomas con su hora de expiración. Al terminar, desmarcarla con `npm run task:release -- --task {ID}`. **`task:release` solo libera la tarea del agente que la tomó** (o un marcado expirado); un agente NUNCA puede liberar la tarea activa de otro — no existe `--force` en `release`, así que un marcado ajeno activo se rechaza hasta que su autor lo libere o expiren las 6 horas. Un marcado sin liberar que supera las **6 horas** se considera olvidado: cualquier agente puede re-tomarlo (`task:take ... --force`, con aviso) o liberarlo (`task:release`, sin force). **Enforcement:** `task:check -- {ID}` **BLOQUEA (exit 78)** el cierre de una tarea tomada por otro agente activo salvo `--allow-foreign` explícito (validación legítima tipo CI de un commit ajeno); la toma propia se **renueva automáticamente** en cada gate (heartbeat: un trabajo largo no expira a mitad) y al cerrar recuerda liberarla. **Visibilidad:** cualquier `task:check`, `run-with-db` o `glory-dev` muestra un banner `EN CURSO` por cada toma ajena activa — no solo la tarea objetivo — para que un agente trabajando su propia tarea sepa que hay trabajo en paralelo sobre el mismo checkout. **Para que el gate reconozca tu propia toma, usa el mismo nombre en `--by` y en `GLORY_AGENT_ID=<agente>` al ejecutar `task:check`**; sin `GLORY_AGENT_ID`, el gate usa el hostname y puede mostrar “tomada por otro” aunque sea tuya. El registro vive en `.quality-reports/task-takeover/` (ignorado por git): es coordinación local del checkout compartido, nunca se commitea.
